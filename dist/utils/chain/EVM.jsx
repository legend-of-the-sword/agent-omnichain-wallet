...
class EVM {
     ...
     */
    async sendSignedTransaction(transaction, signature) {
        try {
     ...
     */
    async handleTransaction(tx, account, keyPath, derivationRootPublicKey) {
        const from = EVM.deriveProductionAddress(account?.accountId, keyPath, derivationRootPublicKey);
        ...
        if (signature) {
            ...
            const transactionResponse = await this.sendSignedTransaction(transaction, ethers.Signature.from({ r, s, v }));
            toast.success(<span>
          View on {this.name}:{" "}
          <Link href={`${this.scanUrl}/tx/${transactionResponse.hash}`} target="_blank" rel="noopener noreferrer">
            Transaction Details
          </Link>
        </span>);
