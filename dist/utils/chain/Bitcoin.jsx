...
export class Bitcoin {
    ...
    async fetchUTXOs(address) {
        try {
            const response = await axios.get(`${this.rpcEndpoint}address/${address}/utxo`);
    ...
    async fetchTransaction(transactionId) {
        const { data } = await axios.get(`${this.rpcEndpoint}tx/${transactionId}`);
     ...
     */
    static deriveProductionAddress(signerId, path, derivationRootPublicKey) {
     ...
     */
    static joinSignature(signature) {
     ...
     */
    async sendTransaction(txHex, options) {
        try {
            const proxyUrl = options?.proxy ? "https://corsproxy.io/?" : "";
            const response = await axios.post(`${proxyUrl}${this.rpcEndpoint}tx`, txHex);
     ...
     */
    async handleTransaction(data, account, keyPath, derivationRootPublicKey) {
        const satoshis = Bitcoin.toSatoshi(data.value);
        const { address, publicKey } = Bitcoin.deriveProductionAddress(account.accountId, keyPath, derivationRootPublicKey);
        const utxos = await this.fetchUTXOs(address);
        ...
        const mpcKeyPair = {
            ...
            sign: async (transactionHash) => {
                const signature = await signMPC(account, Array.from(ethers.getBytes(transactionHash)), keyPath);
                if (!signature) {
        ...
        if (txid) {
            toast.success(<span>
          View on {this.name}:{" "}
          <Link href={`${this.scanUrl}/tx/${txid}`} target="_blank" rel="noopener noreferrer">
            Transaction Details
          </Link>
        </span>);
        }
